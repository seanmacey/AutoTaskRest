...
function Invoke-AutoTaskAPIREST() {
    ...
    function write-CompanyPrimary() {
        ...
        $ChildAlerts = Read-CompanyChildAlerts -CompanyID $CompanyID
        $x = 1
$a = @(1,3)
foreach ($x in $a){
        ...
        if ($alert) {
            ...
            Write-Verbose  "write-CompanyPrimary alertTypeID:$x  initial data exists company $CompanyID and alertType $x and =  $json"
            ...
            $alert.alertText = $alert.alertText  -replace '^(\n)*',"" #-replace "^(`n",""
             if ($alert.alertText){
           $json = $alert | ConvertTo-Json 
            Invoke-AutoTaskAPIREST -url ('V1.0/Companies/' + $CompanyID + '/Alerts') -Method PUT -Body $json |Out-Null
            }
            elseif($alert.id){
                #there is no needed alertText, so DELETE the alert
                Invoke-AutoTaskAPIREST -url ('V1.0/Companies/' + $CompanyID + '/Alerts/' + $alert.id) -Method DELETE  |Out-Null
            }
            ...
            $json = $alert | ConvertTo-Json
            Invoke-AutoTaskAPIREST -url ('V1.0/Companies/' + $CompanyID + '/Alerts') -Method POST -Body $json |Out-Null
        }
    }
